...
func bindLuxUniPKIValid(address common.Address, caller bind.ContractCaller, transactor bind.ContractTransactor, filterer bind.ContractFilterer) (*bind.BoundContract, error) {
	parsed, err := abi.JSON(strings.NewReader(LuxUniPKIValidABI))
	if err != nil {
		return nil, err
	}
...
//
// Solidity: function CheckCert(bytes32 _newHash, address _addrCA, address _addrRoot) constant returns(uint256 _result)
...
//
// Solidity: function DecodeReturnErr(uint256 _res) constant returns(uint256 _err)
...
//
// Solidity: function DecodeReturnIter(uint256 _res) constant returns(uint256 _iter)
...
//
// Solidity: function EncodeReturn(uint256 _iter, uint256 _err) constant returns(uint256 _res)
